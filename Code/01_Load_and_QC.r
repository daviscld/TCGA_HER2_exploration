...
#check metadata for % missingness
lapply(combined_meta, function(x) sum(is.na(x)/length(x))*100)

#subset metadata and samples to only female samples
...
final_meta <- final_meta %>%
    rename(Cancer_Stage = 
                 Neoplasm_Disease_Stage_American_Joint_Committee_on_Cancer_Code)
#make cancer stage into stage I, II, III, IV, unknown
final_meta <- final_meta %>%
    mutate(Cancer_Stage = case_when(
        grepl("Stage I", Cancer_Stage, ignore.case = TRUE) ~ "Stage_I",
        grepl("Stage II", Cancer_Stage, ignore.case = TRUE) ~ "Stage_II",
        grepl("Stage III", Cancer_Stage, ignore.case = TRUE) ~ "Stage_III",
        grepl("Stage IV", Cancer_Stage, ignore.case = TRUE) ~ "Stage_IV",
        TRUE ~ "Unknown"
    ))
...
#Make sure all variables to include in design matrix are factors
#and not NA- change any NA to unknown and set as factor level
meta_deseq <- meta_deseq %>%
  mutate(
    ER_Status_By_IHC = if_else(is.na(ER_Status_By_IHC), "Unknown", ER_Status_By_IHC),
    PR_status_by_ihc = if_else(is.na(PR_status_by_ihc), "Unknown", PR_status_by_ihc),
    Cancer_Stage = if_else(is.na(Cancer_Stage), "Unknown", Cancer_Stage)
  )
meta_deseq$ER_status <- factor(meta_deseq$ER_Status_By_IHC,
                                levels = c("Negative", "Positive", "Indeterminate", "Unknown"))
meta_deseq$PR_status <- factor(meta_deseq$PR_status_by_ihc,
                                levels = c("Negative", "Positive", "Indeterminate", "Unknown"))
meta_deseq$Cancer_Stage <- factor(meta_deseq$Cancer_Stage,
                                levels = c("Stage_I", "Stage_II", "Stage_III", "Stage_IV", "Unknown"))
dds <- DESeqDataSetFromMatrix(
  countData = counts_deseq,
  colData   = meta_deseq,
  design = ~ ER_status + PR_status + Cancer_Stage + her2_clinical
)
